package com.yourorg.telemedicine.ai.service;

import java.time.LocalDateTime;
import java.util.List;


import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;

import com.yourorg.telemedicine.ai.client.WearableClient;
import com.yourorg.telemedicine.ai.dto.VitalReadingDto;
import com.yourorg.telemedicine.ai.entity.ChatMessage;
import com.yourorg.telemedicine.ai.entity.ChatRole;
import com.yourorg.telemedicine.ai.entity.VitalReading;
import com.yourorg.telemedicine.ai.repository.ChatMessageRepository;
import com.yourorg.telemedicine.ai.repository.VitalReadingRepository;
import com.yourorg.telemedicine.ai.util.PromptBuilder;

@Service
public class AIChatServiceImpl implements AIChatService {

    @Autowired
    private ChatMessageRepository chatRepo;

    @Autowired
    private VitalReadingRepository vitalRepo;

    @Autowired
    private LlamaService llama; // ✅ Gemini, not OpenAI

    @Autowired
    private PromptBuilder promptBuilder;
    @Autowired
    private WearableClient wearableClient;

    @Override
    public String processChat(Long patientId, String userMessage) {

        // 1️⃣ Save user message
        chatRepo.save(
            new ChatMessage(patientId, ChatRole.USER, userMessage, LocalDateTime.now())
        );

        // 2️⃣ Fetch & save latest vitals from wearable
        VitalReading vitals = fetchAndSaveVitals(patientId);


        // 3️⃣ Fetch last chat history (ordered)
        List<ChatMessage> history =
            chatRepo.findTop10ByPatientIdOrderByTimestampDesc(patientId)
                    .stream()
                    .sorted((a, b) -> a.getTimestamp().compareTo(b.getTimestamp()))
                    .toList();

        // 4️⃣ Build AI prompt (safe if vitals = null)
        String prompt = promptBuilder.buildPrompt(
                vitals,
                history,
                userMessage
        );

        // 5️⃣ Ask Gemini
        String aiReply = llama.ask(prompt);

        // 6️⃣ Save AI reply
        chatRepo.save(
            new ChatMessage(patientId, ChatRole.AI, aiReply, LocalDateTime.now())
        );

        return aiReply;
    }

    private VitalReading fetchAndSaveVitals(Long patientId) {

        VitalReadingDto dto = wearableClient.getLatestVitals(patientId);

        if (dto == null) return null;

        VitalReading vr = new VitalReading();
        vr.setPatientId(dto.getPatientId());
        vr.setHeartRate(dto.getHeartRate());
        vr.setSpo2(dto.getSpo2());
        vr.setSystolicBP(dto.getSystolicBP());
        vr.setDiastolicBP(dto.getDiastolicBP());
        vr.setTimestamp(LocalDateTime.now());

        return vitalRepo.save(vr);
    }


}
